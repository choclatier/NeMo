...
from nemo.collections.nlp.models.duplex_text_normalization.duplex_tagger import ONNXDuplexTaggerModel
...
@experimental
class ONNXDuplexTextNormalizationModel():
    """
    DuplexTextNormalizationModel is a wrapper class that can be used to
    encapsulate a trained tagger and a trained decoder. The class is intended
    to be used for inference only (e.g., for evaluation).
    ...
    def __init__(self, tagger, encoder, decoder, decoder_init, lang):

        self.tagger = ONNXDuplexTaggerModel(tagger, lang=lang)
        self.decoder = ONNXDuplexDecoderModel(encoder, decoder, decoder_init, lang=lang)
        self.lang = lang
